...
public class Diet {
    ...
    int solveDietProblem(int n, int m, double A[][], double[] b, double[] c, double[] x) {
        ...
        double[][] newA;
        double[] newB;
        List<int[]> combin = combin(IntStream.range(0, n + m + 1).toArray(), m, 0, new int[m], new ArrayList<>());
        for (int[] currentCombination : combin) {
            newA = new double[m][m];
            newB = new double[m];
    ...
    public static List<int[]> combin(int[] arr, int len, int startPosition, int[] combination, List<int[]> result) {
        if (len == 0) {
        ...
        for (int i = startPosition; i <= arr.length - len; i++) {
            ...
            combin(arr, len - 1, i + 1, combination, result);
        }
        return result;
    }
