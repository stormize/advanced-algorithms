...
public class Evacuation {
    ...
    private static int maxFlow(FlowGraph graph, int from, int to) {
        ...
        while(augment){
            Map<Integer,Integer> p=bfs(graph,from,to);
            if(p.get(to)==null)break;
            augment=reflow(graph,to,p);
        }
    ...
    private static boolean reflow(FlowGraph graph,int to,Map<Integer,Integer> prev){
    int current = to;
    ...
    static Map<Integer,Integer> bfs (FlowGraph graph ,int from,int to){
    ...
    while(!queue.isEmpty()){
        ...
        visited[u]=true;
        List<Integer> edges=graph.getIds(u);
        for (Integer v : edges){
            Edge edge =graph.getEdge(v);
