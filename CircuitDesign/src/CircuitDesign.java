...
public class CircuitDesign {
	...
	class TwoSatisfiability {
		int Vars;
		Clause[] clauses;
		ArrayList<HashSet<Integer>> scc = new ArrayList<HashSet<Integer>>();
		...
		void setup(){
			ArrayList<Integer>[] adj = (ArrayList<Integer>[])new ArrayList[2*Vars];
			ArrayList<Integer>[] adjR = (ArrayList<Integer>[])new ArrayList[2*Vars];
			...
			boolean[] visited = new boolean[2*Vars];
			for(int i=0; i<2*Vars; i++){
				if(!visited[i]){
					visited[i] = true;
		...
		boolean isSatisfiable(int[] result) {
			for(HashSet<Integer> scc: scc){
		for(int i: scc){
			if(scc.contains(i+Vars) || scc.contains(i-Vars)) return false;
				}
			}
